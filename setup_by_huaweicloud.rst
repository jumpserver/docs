...
.. code-block:: shell
    ...
    $ echo -e "\033[31m 3. 下载组件 \033[0m" \
      ...
      && pip install wheel -i https://mirrors.huaweicloud.com/repository/pypi/simple \
...
.. code-block:: shell
    ...
    $ echo -e "\033[31m 6. 配置自启 \033[0m" \
      && if [ ! -f "/usr/lib/systemd/system/jms.service" ]; then wget -O /usr/lib/systemd/system/jms.service https://demo.jumpserver.org/download/shell/1.4.8/centos/jms.service; chmod 755 /usr/lib/systemd/system/jms.service; fi \
      && if [ ! -f "/opt/start_jms.sh" ]; then wget -O /opt/start_jms.sh https://demo.jumpserver.org/download/shell/1.4.8/centos/start_jms.sh; fi \
      && if [ ! -f "/opt/stop_jms.sh" ]; then wget -O /opt/stop_jms.sh https://demo.jumpserver.org/download/shell/1.4.8/centos/stop_jms.sh; fi \
