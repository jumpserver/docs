...
- :heart:{: .heart }  视频教程 [JumpServer 从入门到精通](https://www.bilibili.com/video/BV19D4y1S7s4)
...
| Email主题前缀 | [JMS]                       | 邮件的标题，收到的邮件会是 `[JMS]创建用户成功`   |
...
| 发送账号   | 296015668@qq.com | `测试连接` 必须要输入             |
| 测试收件人 | 296015668@qq.com | `测试连接` 必须要输入              |
...
!!! note "点击页面左侧的 `资产管理` - `系统用户` - `创建系统用户` 创建对应协议系统用户"
