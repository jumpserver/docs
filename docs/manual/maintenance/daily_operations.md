...
JumpServer 默认的安装脚本位于 `/opt/jumpserver-ee-版本号-系统架构/jmsctl.sh`。同时，JumpServer 支持命令行工具 `jmsctl`。
...
| `jmsctl down` | 关闭 JumpServer 服务的所有容器 |
| `jmsctl restart` | 重启 JumpServer 服务的所有容器（会删除容器缓存） |
