# 组件设置
!!! tip ""
    - 通过点击页面右上角小齿轮进入 **系统设置** 页面，点击 **组件设置** ，进入组件设置页面。
...
!!! tip ""
    ...
    - 点击某个组件的 **按钮** ，更新组件的命令，存储与录像存储，会话录像默认存放在服务器本地；会话命令默认存放在数据库中，这里可以更改会话录像与会话命令存储到外部存储。如果选择 **null** 则不做保存。

## 3.组件监控
!!! tip ""
    - 组件监控页面可以查看 JumpServer 的所有组件负载状况以及会话数量。
