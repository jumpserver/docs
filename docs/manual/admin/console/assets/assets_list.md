...
!!! tip ""
    ...
    - 部分数据库支持 koko 通过 TLS 加密连接，如 MySQL, SQLSERVER, POSTGRESQL 等。
    - 支持 koko 连接 redis 集群
...
!!! info "云同步任务自动释放行为说明（v4.10.4及以后版本特性）"
    - 开启“云同步任务自动释放”后，如果首次同步选择了地域 A、B，后续修改同步任务仅保留地域 A，再次同步时系统不会自动释放此前从地域 B 同步进来的资产（旧版本会释放）。
    - 如需下线被移除地域的资产，请手动执行释放或在策略中单独配置清理逻辑，避免误删仍在使用的资源。
