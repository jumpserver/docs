...
!!! summary "功能优化 🚀"
    - perf: 优化 Magnus 连接 PostgreSQL 时对预编译命令的审计
    - perf: 优化使用 Go SDK 通过 Magnus 连接 PostgreSQL 的问题
...
!!! summary "功能优化 🚀"
    - perf: 优化 Magnus 数据库端口映射处理逻辑，已经映射的数据库不再重新分配端口
...
!!! summary "功能优化 🚀"
    - perf: 优化 OAuth 2.0 的服务地址参数拼接
!!! success "问题修复 🐛"
    - fix: 修复系统用户存在不正确的私钥导致解析失败问题
...
!!! info "新增功能 🌱"
    - feat: OpenID 支持 PKCE 方式对接
!!! success "问题修复 🐛"
    - fix: 修复 KoKo 连接 Kubernetes 应用失败的问题
    - fix: 修复系统用户使用密钥测试可连接性失败的问题
...
!!! summary "功能优化 🚀"
    - perf: 优化 Web 文件管理中隐藏父级目录选项
    - perf: 优化用户重置密码时短信验证码为纯数字
    - perf: 优化支持系统用户使用 Ed25519 SSH Key 密钥类型
    - perf: 优化支持用户使用 OpenSSH 9.0 版本客户端连接 KoKo 组件

!!! success "问题修复 🐛"
    - fix: 解决 SQL Server 2008 R2 无法连接的问题【KoKo】
...
!!! success "问题修复 🐛"
    - fix: 修复使用单机部署 Redis 服务时页面打开 Celery 任务日志没有输出的问题
    - fix: 修复连接 MySQL 数据库时不能输入中文的问题 (Koko)
    - fix: 修复通过网关连接 MongoDB 数据库集群时失败的问题 (Koko)
...
!!! info "新增功能 🌱"
    - feat: 支持在操作日志中查看资源的变更信息
    - feat: 支持部署使用 Redis 哨兵集群作为后端缓存数据库
    - feat: 支持自定义 MFA 多因子认证逻辑
    - feat: 支持全局设置连接图形化资产的默认分辨率
    - feat: 支持金山云资产同步【企业版】
    - feat: 支持纳管 ClickHouse 数据库【企业版】
    - feat: 支持手机短信找回用户密码【企业版】
    - feat: 云资产同步支持选择 IP 类型【企业版】

!!! summary "功能优化 🚀"
    - perf: 优化系统设置中对资产、应用授权申请工单的默认有效时间支持按照天数和小时进行设置
    - perf: 优化开放一些资源的批量删除功能，如：标签列表、网域列表、命令过滤器等
    - perf: 优化通过 SSH 终端连接 KoKo 时显示资产的平台字段
    - perf: 优化支持通过网域网关对数据库应用进行批量改密【企业版】
    - perf: 优化支持具有超级工单权限的用户可以为其他用户提交工单申请【企业版】

!!! success "问题修复 🐛"
    - fix: 修复更新第三方用户后用户登录时会重定向到更新密码页面的问题
    - fix: 修复通过 Web Terminal 连接 Windows 资产时最大化显示不全的问题
    - fix: 修复 Celery 任务中时间显示有时区误差的问题
    - fix: 修复由于端口数量限制导致数据库不能删除的问题
    - fix: 修复 Redis 服务单独重启内部发布订阅机制失效的问题，如：系统设置修改不生效
    - fix: 修复部署多节点时，改密邮件偶尔发送失败的问题【企业版】
    - fix: 修复通过 RDP 客户端连接 Windows 资产时窗口标题显示中文乱码的问题【企业版】
